...
const ORPHAN_BRANCH_FINGERPRINT = 'domain.apply-orchestrator.orphan-branch'
const IMAGE_UPLOAD_FAILED_FINGERPRINT =
  'domain.apply-orchestrator.image-upload-failed'
const PUBLISH_FAILED_FINGERPRINT = 'domain.apply-orchestrator.publish-failed'
...
export async function apply(
  ...
  const imageMapResult = await ResultAsync.fromPromise(
    deps.imageProcessor.uploadAll(imageInputs),
    (e) => e,
  )
  if (imageMapResult.isErr()) {
    captureWithFingerprint(
      imageMapResult.error,
      IMAGE_UPLOAD_FAILED_FINGERPRINT,
      { extras: { docIds } },
    )
    return {
      kind: 'failed',
      code: 'ImageUploadFailed',
      message: toMessage(imageMapResult.error),
    }
  }
  const imageMap: ImageUrlMap = imageMapResult.value
  ...
  for (const item of eligible) {
    ...
    let filename = item.publishedFilename
    if (filename === '') {
      const generated = generatePublishedFilename(frontmatter, { applyTime })
      if (generated.isErr()) {
        return {
          kind: 'failed',
  ...
  const createBranchResult = await ResultAsync.fromPromise(
    deps.github.createBranch(deps.defaultBranch, branch),
    (e) => e,
  )
  if (createBranchResult.isErr()) {
    captureWithFingerprint(
      createBranchResult.error,
      PUBLISH_FAILED_FINGERPRINT,
      { extras: { method: 'createBranch', branch } },
    )
    return {
      kind: 'failed',
      code: 'GitHubApiError',
      message: toMessage(createBranchResult.error),
    }
  }
  ...
  if (publishResult.isErr()) {
    captureWithFingerprint(publishResult.error, PUBLISH_FAILED_FINGERPRINT, {
      extras: { method: 'commitFiles|createPullRequest', branch },
    })
    const cleanupResult = await ResultAsync.fromPromise(
      deps.github.deleteBranch(branch),
      (e) => e,
    )
    ...
    return {
      kind: 'failed',
      code: 'GitHubApiError',
      message: toMessage(publishResult.error),
    }
  }

  const pr: BlogPrSummary = publishResult.value
  return {
    kind: 'success',
    prNumber: pr.number,
    prUrl: pr.url,
